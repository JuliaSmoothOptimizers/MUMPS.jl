...
Both low-level interface with MUMPS parallel direct solver C-library
...
and right hand sides directly, so, for example, the equation `Ax = y`, solved
in Base by `x = A \\ y` or `LinearAlbegra.ldiv!(x, A, y)`, can be solved in MUMPS.jl
